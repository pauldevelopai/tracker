...
export default function AgentChatPanel({ agentType, placeholder, emptyText, contextData }) {
  ...
  function loadConversations() {
    apiFetch(`/agent-conversations?agent_type=${agentType}`).then(setConversations).catch(() => setConversations([]));
  }

  useEffect(loadConversations, [agentType]);
  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [activeConv?.messages]);

  async function createConversation() {
    const conv = await apiFetch('/agent-conversations', {
      method: 'POST', body: JSON.stringify({ agent_type: agentType, title: 'New conversation', context: contextData || {} })
    });
  ...
  async function loadConversation(id) {
    const conv = await apiFetch(`/agent-conversations/${id}`);
    setActiveConv(conv);
  }
  ...
  async function sendMsg(e) {
    ...
    setSending(true);
    try {
      const updated = await apiFetch(`/agent-conversations/${activeConv.id}/message`, {
  ...
  return (
    <div style={{ display: 'flex', gap: 16, minHeight: 500 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
        {!activeConv ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', padding: 40, textAlign: 'center' }}>
            <div>
              <div style={{ fontSize: 15, fontWeight: 500, marginBottom: 8 }}>{emptyText || 'Start a conversation'}</div>
              <div style={{ fontSize: 13 }}>Click "+ New Chat" to begin.</div>
            </div>
          </div>
        ) : (
          <>
            <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
              {activeConv.messages.length === 0 && (
                <div style={{ textAlign: 'center', color: 'var(--text-secondary)', paddingTop: 40 }}>
                  <div style={{ fontSize: 13 }}>{placeholder || 'Ask me anything...'}</div>
                </div>
              )}
              {activeConv.messages.map((msg, i) => (
                <div key={i} style={{
                  marginBottom: 12, padding: '10px 14px', borderRadius: 8, maxWidth: '85%',
            ...
            <form onSubmit={sendMsg} style={{ display: 'flex', gap: 8, padding: 12, borderTop: '1px solid var(--border-color)' }}>
              ...
              <button type="submit" className="btn btn-primary btn-small" disabled={sending || !message.trim()}>Send</button>
            </form>
          </>
        )}
      </div>
